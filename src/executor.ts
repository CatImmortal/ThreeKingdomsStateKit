...
function 应用军队战创建(state: 状态总表, command: Extract<状态命令, { type: 'CreateArmyBattle' }>): void {
  if (state.军队战[command.名称]) {
    throw new Error(`军队战已存在: ${command.名称}`);
  }
  const { data } = command;
  ...
  state.军队战[command.名称] = create军队战({
    名称: command.名称,
...
function 应用军队战更新(state: 状态总表, command: Extract<状态命令, { type: 'UpdateArmyBattle' }>): void {
  const battle = 断言存在(state.军队战[command.名称], `军队战不存在: ${command.名称}`);
  if (battle.阶段 === '结束') {
  ...
  if (battle.阶段 === '结束') {
    ...
    delete state.军队战[command.名称];
    return;
  }

  state.军队战[command.名称] = create军队战(battle);
}
