...
function 断言存在<T>(value: T | null | undefined, message: string): NonNullable<T> {
  if (!value) {
...
function 应用势力更新(state: 状态总表, command: Extract<状态命令, { type: 'UpsertFaction' }>): void {
...
function 应用军队战创建(state: 状态总表, command: Extract<状态命令, { type: 'CreateArmyBattle' }>): void {
  if (state.军队战 && state.军队战.阶段 !== '结束') {
    throw new Error(`当前已有进行中的军队战: ${state.军队战.名称}`);
  }
  const { data } = command;
  const 攻方faction = 断言存在(state.势力[data.攻方势力], `攻方势力不存在: ${data.攻方势力}`);
  const 守方faction = 断言存在(state.势力[data.守方势力], `守方势力不存在: ${data.守方势力}`);

  const 攻方军队: Record<string, 参战军队引用> = {};
  for (const armyName of data.攻方军队) {
    断言存在(攻方faction.军队[armyName], `攻方军队不存在: ${data.攻方势力}.${armyName}`);
    攻方军队[armyName] = create参战军队引用({ 名称: armyName, 势力名称: data.攻方势力, 状态: '参战' });
  }
  const 守方军队: Record<string, 参战军队引用> = {};
  for (const armyName of data.守方军队) {
    断言存在(守方faction.军队[armyName], `守方军队不存在: ${data.守方势力}.${armyName}`);
    守方军队[armyName] = create参战军队引用({ 名称: armyName, 势力名称: data.守方势力, 状态: '参战' });
  }

  const 初始阶段 = data.类型 === '攻城战' ? '围城' : '伏击';

  if (data.类型 === '攻城战' && !data.攻城数据?.目标城池) {
    throw new Error(`攻城战必须提供攻城数据: ${command.名称}`);
  }

  let 攻城数据 = undefined;
  if (data.类型 === '攻城战' && data.攻城数据?.目标城池) {
    const cityName = data.攻城数据.目标城池;
    攻城数据 = create攻城数据({
      目标城池: cityName,
      围城状态: data.攻城数据.围城状态 || '未围城',
      器械: data.攻城数据.器械,
    });
  }
...
function 应用军队战更新(state: 状态总表, command: Extract<状态命令, { type: 'UpdateArmyBattle' }>): void {
  const currentBattle = 断言存在(state.军队战, `军队战不存在: ${command.名称}`);
  if (currentBattle.名称 !== command.名称) {
    throw new Error(`当前军队战名称不匹配: ${command.名称}`);
  }
  const battle = _.cloneDeep(currentBattle);
  if (battle.阶段 === '结束') {
    throw new Error(`军队战已结束: ${command.名称}`);
  }

  if (command.changes) {
    if (command.changes.阶段 !== undefined) battle.阶段 = command.changes.阶段 as 军队战['阶段'];
    if (command.changes.遭遇战当前轮次 !== undefined) battle.遭遇战当前轮次 = Math.max(0, 数值(command.changes.遭遇战当前轮次));
    if (command.changes.攻城战当前天数 !== undefined) battle.攻城战当前天数 = Math.max(0, 数值(command.changes.攻城战当前天数));
    if (command.changes.攻城数据 !== undefined) {
      battle.攻城数据 = create攻城数据(合并对象(_.cloneDeep(battle.攻城数据 ?? {}), command.changes.攻城数据));
    }
  }

  if (command.攻方军队) {
    for (const [name, patch] of Object.entries(command.攻方军队)) {
      const existing = battle.攻方军队[name];
      if (existing) {
        battle.攻方军队[name] = create参战军队引用(合并对象(_.cloneDeep(existing), patch));
      }
    }
  }
  if (command.守方军队) {
    for (const [name, patch] of Object.entries(command.守方军队)) {
      const existing = battle.守方军队[name];
      if (existing) {
        battle.守方军队[name] = create参战军队引用(合并对象(_.cloneDeep(existing), patch));
      }
    }
  }

  if (battle.阶段 === '结束') {
    for (const ref of [...Object.values(battle.攻方军队), ...Object.values(battle.守方军队)]) {
      if (ref.状态 !== '全灭') continue;
      const faction = state.势力[ref.势力名称];
      if (!faction?.军队) continue;
      delete faction.军队[ref.名称];
    }
    state.军队战 = null;
    return;
  }

  state.军队战 = create军队战(battle);
}
    ...
    for (const [index, command] of commands.entries()) {
      ...
      switch (command.type) {
        ...
        case 'CreateArmyBattle':
          应用军队战创建(next, command);
          break;
        case 'UpdateArmyBattle':
          应用军队战更新(next, command);
          break;
      }
