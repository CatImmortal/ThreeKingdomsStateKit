<template>
  <section class="tk-panel-card">
    ...
    <div v-if="currentFaction" class="tk-panel-inline-note">当前展示势力：{{ currentFaction.名称 || currentFactionId }}（{{ currentFactionId }}）</div>
    <div class="tk-panel-detail-list">
      <details v-for="[id, army] in armies" :key="id" class="tk-panel-detail">
        <summary><span>{{ id }}</span><span>{{ army.兵种 }} / {{ army.等级 }}</span></summary>
        <div class="tk-panel-detail-body">
          ...
          <div class="tk-panel-kv-grid compact">
            ...
            <div class="tk-panel-kv"><span class="tk-panel-k">攻击战力</span><span class="tk-panel-v">{{ army._攻击战力 ?? 0 }}</span></div>
            <div class="tk-panel-kv"><span class="tk-panel-k">防御战力</span><span class="tk-panel-v">{{ army._防御战力 ?? 0 }}</span></div>
          </div>
        </div>
      </details>
...
const currentFactionId = computed(() => currentFactionEntry.value?.[0] ?? '');
