<template>
  <div v-if="currentFaction" class="tk-panel-page-grid cols-2">
    <section class="tk-panel-card">
      ...
      <div class="tk-panel-inline-note">当前展示势力：{{ currentFaction.名称 || currentFactionId }}（{{ currentFactionId }}）</div>
    ...
    <section class="tk-panel-card cols-span-2">
      ...
      <div class="tk-panel-list">
        <div v-for="[id, city] in cities" :key="id" class="tk-panel-list-item">
          <div class="tk-panel-list-title">{{ id }}</div>
...
const diplomacyItems = computed(() => Object.entries(currentFaction.value?.外交 || {}).slice(0, 8).map(([name, value]) => ({ label: name, value: `${value} / ${currentFaction.value?._外交等级?.[name] || '未知'}` })));
