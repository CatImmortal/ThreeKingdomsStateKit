...
export function create装备条目(data: Partial<装备条目> = {}): 装备条目 {
  return {
    名称: String(data.名称 || ''),
    品质: 枚举.装备品质.includes(data.品质 as 装备品质) ? (data.品质 as 装备品质) : '凡品',
    类型: String(data.类型 || ''),
    描述: String(data.描述 || ''),
    伤害骰: String(data.伤害骰 || ''),
    先攻加值: 数值(data.先攻加值),
...
export function create装备栏(data: Partial<装备栏> = {}): 装备栏 {
  const toSlot = (value: 装备栏位 | undefined): 装备栏位 => (value && value !== '无' ? create装备条目(value) : '无');
  return {
    主武器: toSlot(data.主武器),
    副武器: toSlot(data.副武器),
    护甲: toSlot(data.护甲),
...
export function create兵种适性表(data: Partial<兵种适性表> = {}): 兵种适性表 {
  return Object.fromEntries(枚举.基础兵种.map(兵种 => [兵种, 限制数值(data[兵种], 0, 100)])) as 兵种适性表;
}
