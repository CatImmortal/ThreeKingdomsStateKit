...
export function recompute战斗数据(data: 战斗数据, 六维数据: 六维): 战斗数据 {
  ...
  next._先攻基础值 = Math.floor((六维数据._武力加值 ?? 0) / 4) + 装备汇总.先攻 + (next._伤势减值 ?? 0);
  next._攻击基础值 = (六维数据._武力加值 ?? 0) + 装备汇总.攻击 + (next._伤势减值 ?? 0);
  next._伤害基础值 = (六维数据._武力加值 ?? 0) + (next._伤势减值 ?? 0);
  next._防御DC基础值 = 40 + Math.floor((六维数据._武力加值 ?? 0) * 0.75) + 装备汇总.防御DC + (next._伤势减值 ?? 0);
