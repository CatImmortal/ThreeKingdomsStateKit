...
const 主角字段 = ['六维', '当前生命值', '当前体力值', '生命值上限加成', '体力值上限加成', '装备', '武技', '专长', '状态', '物品栏', '兵种适性', '声望', '金钱', '积分', '官职', '爵位', '后宫和谐度'] as const;
...
function 校验角色战斗数据(value: unknown, path: string): void {
  ...
  if (value.生命值上限加成 !== undefined) 断言数字(value.生命值上限加成, `${path}.生命值上限加成`);
  if (value.体力值上限加成 !== undefined) 断言数字(value.体力值上限加成, `${path}.体力值上限加成`);
...
function 校验物品栏(value: unknown, path: string): void {
  ...
  for (const [key, item] of Object.entries(value)) {
    ...
    if (item.名称 !== undefined) 断言字符串(item.名称, `${path}.${key}.名称`);
