...
export type 参战军队引用更新 = Partial<基础字段<参战军队引用>>;
export type 攻城数据更新 = Partial<基础字段<攻城数据>>;
...
export type 状态命令 =
  ...
  | {
      ...
      攻方军队?: Record<string, 参战军队引用更新>;
      守方军队?: Record<string, 参战军队引用更新>;
    };
...
const NPC字段 = ['名称', '品质', '所属势力', '定位', '所在地', '好感', '简述', '羁绊', '六维', '战斗数据', '武将信息', '美人属性'] as const;
const 武将信息字段 = ['野心值', '性格', '官职', '当前状态', '状态描述', '忠诚', '是否已招募', '兵种适性', '特技'] as const;
...
const 参战军队引用字段 = ['名称', '势力名称', '状态'] as const;
const 攻城数据字段 = ['目标城池', '围城状态', '器械'] as const;
const 军队战创建字段 = ['类型', '地形', '攻方势力', '守方势力', '攻方军队', '守方军队', '攻城数据'] as const;
const 军队战更新字段 = ['阶段', '遭遇战当前轮次', '攻城战当前天数', '攻城数据'] as const;
...
function 校验参战军队引用更新(value: unknown, path: string): void {
  断言非空对象(value, path);
  断言字段白名单(value, 参战军队引用字段, path);
  if (value.名称 !== undefined) 断言字符串(value.名称, `${path}.名称`);
  if (value.势力名称 !== undefined) 断言字符串(value.势力名称, `${path}.势力名称`);
  if (value.状态 !== undefined) 断言枚举值(value.状态, 枚举.参战军队状态, `${path}.状态`);
}

function 校验参战军队引用映射(value: unknown, path: string): void {
  断言对象(value, path);
  for (const [key, item] of Object.entries(value)) {
    断言字符串(key, `${path} 键名`);
    校验参战军队引用更新(item, `${path}.${key}`);
  }
}

function 校验攻城数据更新(value: unknown, path: string): void {
  断言对象(value, path);
  断言字段白名单(value, 攻城数据字段, path);
  if (value.目标城池 !== undefined) 断言字符串(value.目标城池, `${path}.目标城池`);
  if (value.围城状态 !== undefined) 断言枚举值(value.围城状态, 枚举.围城状态, `${path}.围城状态`);
  if (value.器械 !== undefined) 校验数字映射(value.器械, `${path}.器械`);
}

function 校验军队战创建数据(value: unknown, path: string): void {
  断言非空对象(value, path);
  断言字段白名单(value, 军队战创建字段, path);
  断言枚举值(value.类型, 枚举.军队战类型, `${path}.类型`);
  断言字符串(value.地形, `${path}.地形`);
  断言字符串(value.攻方势力, `${path}.攻方势力`);
  断言字符串(value.守方势力, `${path}.守方势力`);
  断言(Array.isArray(value.攻方军队), `${path}.攻方军队 必须是数组`);
  断言(value.攻方军队.length > 0, `${path}.攻方军队 至少包含一支军队`);
  (value.攻方军队 as unknown[]).forEach((item, index) => 断言字符串(item, `${path}.攻方军队[${index}]`));
  断言(Array.isArray(value.守方军队), `${path}.守方军队 必须是数组`);
  断言(value.守方军队.length > 0, `${path}.守方军队 至少包含一支军队`);
  (value.守方军队 as unknown[]).forEach((item, index) => 断言字符串(item, `${path}.守方军队[${index}]`));
  if (value.类型 === '攻城战') {
    断言(value.攻城数据 !== undefined, `${path}.攻城数据 在攻城战中必填`);
    const 攻城数据 = value.攻城数据 as Record<string, unknown>;
    校验攻城数据更新(攻城数据, `${path}.攻城数据`);
    断言(String(攻城数据.目标城池 || '').trim().length > 0, `${path}.攻城数据.目标城池 在攻城战中必填`);
    return;
  }
  if (value.攻城数据 !== undefined) 校验攻城数据更新(value.攻城数据, `${path}.攻城数据`);
}

function 校验军队战更新(value: unknown, path: string): void {
  断言非空对象(value, path);
  断言字段白名单(value, 军队战更新字段, path);
  if (value.阶段 !== undefined) 断言枚举值(value.阶段, 枚举.军队战阶段, `${path}.阶段`);
  if (value.遭遇战当前轮次 !== undefined) 断言数字(value.遭遇战当前轮次, `${path}.遭遇战当前轮次`);
  if (value.攻城战当前天数 !== undefined) 断言数字(value.攻城战当前天数, `${path}.攻城战当前天数`);
  if (value.攻城数据 !== undefined) 校验攻城数据更新(value.攻城数据, `${path}.攻城数据`);
}
  ...
  try {
    switch (command.type) {
      ...
      case 'RemoveShopItem':
        ...
        断言字符串(command.名称, `${path}.名称`);
        return;
      case 'CreateArmyBattle':
        debugLog('commands', '校验 CreateArmyBattle', { index, 名称: command.名称 });
        断言字符串(command.名称, `${path}.名称`);
        校验军队战创建数据(command.data, `${path}.data`);
        return;
      case 'UpdateArmyBattle':
        debugLog('commands', '校验 UpdateArmyBattle', { index, 名称: command.名称 });
        断言字符串(command.名称, `${path}.名称`);
        断言(command.changes !== undefined || command.攻方军队 !== undefined || command.守方军队 !== undefined, `${path} 至少要提供 changes、攻方军队、守方军队 之一`);
        if (command.changes !== undefined) 校验军队战更新(command.changes, `${path}.changes`);
        if (command.攻方军队 !== undefined) 校验参战军队引用映射(command.攻方军队, `${path}.攻方军队`);
        if (command.守方军队 !== undefined) 校验参战军队引用映射(command.守方军队, `${path}.守方军队`);
        return;
    }
  } catch (error) {
