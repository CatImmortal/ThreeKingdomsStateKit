...
- 状态持久化（将状态快照写入 assistant 聊天楼层的 `data` 字段，跨楼层断点续读；删除最新楼层后可回退到上一个可用快照）
...
构建工具链：tsup + esbuild + unplugin-vue（Vue SFC 编译器）。当前构建会直接产出单个压缩后的 `dist/index.js`，以适配酒馆助手脚本只支持单文件注入的限制。
...
当前全局对象已收缩为以运行时核心能力为主的公开面：保留状态创建、协议解析、AI 回复处理、事件接线、系统界面开关与调试开关；不再继续透传完整 `rules / commands / storage / executor / context / dom-host / recompute` 模块全集，以降低单文件 bundle 体积。
...
- assistant 消息正文不再附加 `<StatusBar>` 或 `<PlayerOptionsPanel>` 标记；但会保留原始 `<PlayerOptions>` 文本，供宿主外部正则隐藏
...
- AI 回复处理完成后只同步最新状态到 `src/ui/store.ts`，不会自动弹出系统界面
- 玩家选项显示在独立 Vue 玩家选项悬浮窗中，并通过 `handlePlayerOptionClick()` 填入宿主输入栏
- 玩家选项窗的显隐不再只依赖单次 AI 处理结果，而是由 `MESSAGE_SENT`、`MESSAGE_RECEIVED`、`MESSAGE_DELETED`、`CHAT_CHANGED` 事件驱动，并按“当前最新楼层是否仍含有效 `<PlayerOptions>`”动态刷新
